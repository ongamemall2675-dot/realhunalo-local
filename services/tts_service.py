...
class TTSService:
    ...
    # Fallback 순서 정의
    FALLBACK_ORDER = ["elevenlabs", "azure"]
    ...
    def __init__(self):
        ...
        # 기본 엔진 설정 (ElevenLabs 우선, 없으면 Azure)
    ...
    def generate(
        ...
        **kwargs
        ...
        Args:
            ...
            enable_fallback: True면 실패 시 다른 엔진으로 자동 전환
            **kwargs: 엔진별 추가 옵션
        ...
        # 시도할 엔진 목록 구성
        engines_to_try = self._get_engines_to_try(target_engine_name, enable_fallback)
        ...
        for engine_name in engines_to_try:
            ...
            if engine_name != target_engine_name:
                ...
                print(f"[WARN] Fallback: {target_engine_name} → {engine_name}")
            ...
            try:
                ...
                if result.success:
                    ...
                    if engine_name != target_engine_name:
                        ...
                        response["originalEngine"] = target_engine_name
        ...
        return {
            "success": False,
            "error": f"모든 TTS 엔진 실패. 마지막 오류: {last_error}",
            "sceneId": scene_id,
            "triedEngines": engines_to_try
        }

    def _get_engines_to_try(self, requested: str, enable_fallback: bool) -> List[str]:
        """시도할 엔진 목록 반환"""
        if not enable_fallback:
            # Fallback 비활성화 시 요청한 엔진만
            if requested and requested in self.engines:
                return [requested]
            return []

        # Fallback 활성화: 요청 엔진 우선, 그 다음 나머지
        engines = []
        ...
        for engine in self.FALLBACK_ORDER:
            if engine in self.engines and engine not in engines:
